...
mock.module("./services/pipeline.js", () => ({
  runDailyWorkflowAndPersist: (...a: unknown[]) => pipeline.runDailyWorkflowAndPersist(...a),
}));
...
mock.module("./services/engagement.js", () => ({
  processEngagementEvent: (...a: unknown[]) => engagement.processEngagementEvent(...a),
}));
...
beforeEach(() => {
  ...
  publisher.publishDuePosts = async () => ({ processed: 0, skipped: 0, failed: [] });
...
describe("POST /cron/execute-post — scan mode", () => {
  ...
  it("calls publishDuePosts and returns result", async () => {
    publisher.publishDuePosts = mock(async () => ({ processed: 1, skipped: 0, failed: [] }));
...
describe("POST /cron/execute-post — single-post mode", () => {
  ...
  it("returns 200 with skipped status when post already claimed", async () => {
    publisher.publishSinglePost = mock(async () => ({ ok: true, status: "skipped" }));
    const res = await singlePost(10);
    expect(res.status).toBe(200);
...
describe("GET /webhooks/x", () => {
  it("returns sha256 CRC response for valid crc_token", async () => {
    ...
    const expectedToken =
      "sha256=" + createHmac("sha256", WEBHOOK_SECRET).update("abc123").digest("base64");
