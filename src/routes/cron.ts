...
cron.post("/execute-post", async (c) => {
  ...
  if (!result.ok) return c.json({ ok: false, error: result.error }, result.httpStatus);
  if (result.status === "skipped") return c.json({ ok: true, status: "skipped" });
  return c.json({ ok: true, tweetId: result.tweetId, tweetUrl: result.tweetUrl });
});
