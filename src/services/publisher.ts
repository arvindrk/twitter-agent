...
type PublishSuccess = { ok: true; status: "published"; tweetId: string; tweetUrl: string };
...
export async function publishSinglePost(postId: number): Promise<SinglePostResult> {
...
export async function publishDuePosts(): Promise<ScanResult> {
  ...
  for (const { id } of due) {
    ...
    const result = await execPublish(post);
    if (result.ok) { processed++; } else { failed.push({ id, error: result.error }); }
  }
