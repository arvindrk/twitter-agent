...
describe("runResearcher", () => {
  ...
  it("passes userMessage as the user message content", async () => {
    ...
    const lastCall = (mockGenerateText.mock.calls.at(-1) as unknown[])[0] as { messages: { role: string; content: string }[] };
    expect(lastCall.messages[0]).toEqual({ role: "user", content: "test query" });
  });
  ...
  it("propagates errors from generateText", async () => {
    ...
    await expect(runResearcher("anything")).rejects.toThrow("API rate limit");
  });
});
