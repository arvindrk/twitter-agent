...
- **[Xquik](https://xquik.com)** - optional API key backend for text posts and replies
...
| `TWITTER_BACKEND`       | Write backend: `x` by default or `xquik`                                               |
| `XQUIK_API_KEY`         | Xquik API key, required when `TWITTER_BACKEND=xquik`                                   |
| `XQUIK_ACCOUNT`         | Connected Xquik account as `@username` or account ID                                   |
| `XQUIK_BASE_URL`        | Xquik REST base URL, defaults to `https://xquik.com/api/v1`                            |
